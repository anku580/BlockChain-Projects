      ...
      countBlocks()
        .then((count) => {
          ...
          let interval = setInterval(function () {
            ...
            self.getBlock(i)
              .then((block) => {
                let blockToRead = JSON.parse(block);
                //console.log(blockToRead.body);
                if (blockToRead.body.address === staraddress) {
                  let star1 = JSON.parse(block);

                  let encodedValue = star1.body.star.story;
                  //console.log(star1.body.star.story);
                  let decodedValue = Buffer(encodedValue, 'hex').toString();
                  //console.log(decodedValue);
                  star1.body.star.storyDecoded = decodedValue;
          ...
          setTimeout(() => resolve(starArray), 200 * (count + 1));
...
addDataToLevelDB = (value) => {
  ...
  db.createReadStream().on('data', function (data) {
    ...
    // addLevelDBData(value.hash, JSON.stringify(value));
  });
}
